...
class PGAgent(object):
  ...
  # Actor.run() explore_samples, policy_samples
  ...
  # Learner.run()
  def train(self, samples, back_translation_reward, use_baseline=False,
            debug=True, parameters=None, min_prob=0.0, scale=1.0,
  ...
  # Learner.run() -> train -> compute_probs compute traj's prob
  ...
  # experiment.py Evaluator.run() and Learner.run()
...
class AllGoodReplayBuffer(ReplayBuffer):
  ...
  def save(self, samples):
    trajs = [s.traj for s in samples]
    self.save_trajs_new(samples, trajs)

  def save_trajs_new(self, samples, trajs):
  ...
  def replay(self, envs, n_samples=1, use_top_k=False, agent=None, truncate_at_n=0):
    ...
    for name, samples in env_sample_dict.iteritems():
      ...
      if truncate_at_n > 0 and n > truncate_at_n:
        ...
        # headpq.nlargest -- find the N largest elements
...
def sampling(model, envs, temperature=1.0, use_encode=True,
             greedy=False, n_samples=1, debug=True, use_cache=False,
             filter_error=True):
...
def beam_search(model, envs, use_encode=True, beam_size=1, debug=False, renorm=True,
  ...
  # Collect the training examples. h in beam
