...
# Experiment name
tf.flags.DEFINE_string('output_dir', '/Users/qingwang/PycharmProjects/data/wikisql/output', 'output folder.')
tf.flags.DEFINE_string('experiment_name', 'eval_mml_test',
...
tf.app.flags.DEFINE_bool(
  'use_cache', True, 'Use cache to avoid generating the same samples.')
tf.app.flags.DEFINE_string(
  'en_vocab_file', '/Users/qingwang/PycharmProjects/data/wikisql/processed_input/preprocess_2/en_vocab_min_count_5.json', '.')
tf.app.flags.DEFINE_string(
  'executor', 'wikisql', 'Which executor to use, wtq or wikisql.')
...
tf.app.flags.DEFINE_integer(
  'attn_size', 50, 'Size of attention vector.')
tf.app.flags.DEFINE_integer(
  'attn_vec_size', 50, 'Size of the vector parameter for computing attention.')
tf.app.flags.DEFINE_integer(
  'n_layers', 1, 'Number of layers in decoder.')
tf.app.flags.DEFINE_integer(
  'en_n_layers', 1, 'Number of layers in encoder.')
tf.app.flags.DEFINE_integer(
  'en_embedding_size', 50, 'Size of encoder input embedding.')
tf.app.flags.DEFINE_integer(
  'value_embedding_size', 300, 'Size of value embedding for the constants.')
tf.app.flags.DEFINE_bool(
  'en_bidirectional', True, 'Whether to use bidirectional RNN in encoder.')
tf.app.flags.DEFINE_bool(
  'en_attn_on_constants', False, '.')
tf.app.flags.DEFINE_bool(
  'use_pretrained_embeddings', True, 'Whether to use pretrained embeddings.')
tf.app.flags.DEFINE_integer(
...
# Data
tf.app.flags.DEFINE_string(
  'table_file', '/Users/qingwang/PycharmProjects/data/wikisql/processed_input/preprocess_2/tables.jsonl', 'Path to the file of wikitables, a jsonl file.')
tf.app.flags.DEFINE_string(
  'train_file', '', 'Path to the file of training examples, a jsonl file.')
tf.app.flags.DEFINE_string(
  'dev_file', '/Users/qingwang/PycharmProjects/data/wikisql/processed_input/preprocess_2/train_split_shard_30-0.jsonl', 'Path to the file of training examples, a jsonl file.')
tf.app.flags.DEFINE_string(
  'eval_file', '/Users/qingwang/PycharmProjects/data/wikisql/processed_input/preprocess_2/dev_split.jsonl', 'Path to the file of test examples, a jsonl file.')
tf.app.flags.DEFINE_string(
  'embedding_file', '/Users/qingwang/PycharmProjects/data/wikisql/raw_input/wikisql_glove_embedding_mat.npy', 'Path to the file of pretrained embeddings, a npy file.')
tf.app.flags.DEFINE_string(
  'vocab_file', '/Users/qingwang/PycharmProjects/data/wikisql/raw_input/wikisql_glove_vocab.json', 'Path to the vocab file for the pretrained embeddings, a json file.')
tf.app.flags.DEFINE_string(
  'train_shard_dir', '/Users/qingwang/PycharmProjects/data/wikisql/processed_input/preprocess_2/', 'Folder containing the sharded training data.')
tf.app.flags.DEFINE_string(
  'train_shard_prefix', 'train_split_shard_30-', 'The prefix for the sharded files.')
tf.app.flags.DEFINE_integer(
...
tf.app.flags.DEFINE_integer(
  'shard_end', 1, 'End id of the shard to use.')
...
tf.app.flags.DEFINE_string(
  'saved_program_file', '/Users/qingwang/PycharmProjects/data/wikisql/processed_input/preprocess_2/all_train_saved_programs-1k.json', 'Saved program file.')
...
tf.app.flags.DEFINE_integer(
  'batch_size', 25, 'Model batch size.')
tf.app.flags.DEFINE_integer(
  'n_actors', 1, 'Number of actors for generating samples.')
tf.app.flags.DEFINE_integer(
  'save_every_n', 10,
...
tf.app.flags.DEFINE_bool(
  'use_nonreplay_samples_in_train', True,
...
tf.app.flags.DEFINE_bool(
  'debug', True, 'Whether to output debug information.')


# Device placement.
tf.app.flags.DEFINE_bool(
  'train_use_gpu', True, 'Whether to output debug information.')
tf.app.flags.DEFINE_integer(
  'train_gpu_id', 0, 'Id of the gpu used for training.')

tf.app.flags.DEFINE_bool(
  'eval_use_gpu', True, 'Whether to output debug information.')
tf.app.flags.DEFINE_integer(
...
def collect_traj_for_program(env, program):
  ...
  for tk in program:
    ...
    try:
      ...
      return None
    ob, _, _, _ = env.step(action, debug=True)
...
def create_envs(table_dict, data_set, en_vocab, embedding_model):
  ...
  if FLAGS.executor == 'wtq':
    ...
    score_fn = utils.wikisql_score # score_fn = 0.0 or 1.0
  ...
  for i, example in enumerate(data_set):
      ...
      kg_info = table_dict[example['context']] # kg: knowledge graph
...
def init_experiment(fns, use_gpu=False, gpu_id='0'):
  ...
  if graph_config:   # for eval
    # If evaluating an saved model, just load its graph
    # config.
    # 2. create agent, graph-config create agent, model(agent)
    ...
    graph_config['core_config'] = dict(
      ...
      add_lm_loss=FLAGS.lm_loss_coeff > 0.0, # rnn language model loss
...
class Actor(multiprocessing.Process):
  ...
  def run(self):
    ...
    current_ckpt = get_init_model_path() # current checkpoint

    env_dict = dict([(env.name, env) for env in envs]) # env.name dev-0:envs
    # for each env, find the traj with probs.
    replay_buffer = agent_factory.AllGoodReplayBuffer(agent, envs[0].de_vocab)

    # Load saved programs to warm start the replay buffer. 
    if FLAGS.load_saved_programs:  # True
      load_programs(
    ...
    while True:
      ...
      for j, batch_dict in enumerate(env_iterator):
        ...
        if FLAGS.use_replay_prob_as_weight: # FLAGS.use_replay_prob_as_weight=false
          new_samples = []
          for sample in replay_samples:
            name = sample.traj.env_name
            if name in replay_buffer.prob_sum_dict:
              replay_prob = max(
                replay_buffer.prob_sum_dict[name], FLAGS.min_replay_weight) # FLAGS.min_replay_weight=0.1
            else:
              replay_prob = 0.0
            scale = replay_prob
            new_samples.append(
        ...
        if FLAGS.use_nonreplay_samples_in_train:       # True
          nonreplay_samples = []
        ...
        def weight_samples(samples):
          if FLAGS.use_replay_prob_as_weight:
            ...
            new_samples = agent_factory.scale_probs(
              samples, 1 - FLAGS.fixed_replay_weight) # FLAGS.fixed_replay_weight=0.5
          return new_samples

        train_samples = []
        if FLAGS.use_replay_samples_in_train: # True
          if FLAGS.use_trainer_prob:
            replay_samples = [
              sample._replace(prob=None) for sample in replay_samples]
          train_samples += replay_samples

        FLAGS.use_policy_samples_in_train = True
        if FLAGS.use_policy_samples_in_train:  # False default. In our case, we need it to be true
          train_samples += weight_samples(policy_samples)

        FLAGS.use_nonreplay_samples_in_train = False
        if FLAGS.use_nonreplay_samples_in_train: # True default. In our case, we need it to be false.
        ...
        if FLAGS.use_importance_sampling:    # False
          step_logprobs = agent.compute_step_logprobs(
            [s.traj for s in train_samples])
        else:
          step_logprobs = None

        if FLAGS.use_replay_prob_as_weight:  # False
          n_clip = 0
...
class Evaluator(multiprocessing.Process):
  ...
  def run(self):
    ...
    i = 0
    current_ckpt = get_init_model_path() # ckpt : checkpoint
